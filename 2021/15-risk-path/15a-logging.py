...
class Board:
  ...
  def save_wip(self):
    try:
      os.mkdir(f'{self.filename}.data', 0o755 )
    except FileExistsError:
      'ok'

    log(f"Saving wip")
    numpy.savetxt(f"{self.filename}.data/distances", self.safest_to_here, fmt='%3i')
    numpy.savetxt(f"{self.filename}.data/distances.{self.safest}", self.safest_to_here, fmt='%3i')
    numpy.savetxt(f"{self.filename}.data/path", self.seen, fmt='%i')
    numpy.savetxt(f"{self.filename}.data/path.{self.safest}", self.seen, fmt='%i')


  def load_wip(self):
    self.safest_to_here = numpy.loadtxt(f"{self.filename}.data/distances")
